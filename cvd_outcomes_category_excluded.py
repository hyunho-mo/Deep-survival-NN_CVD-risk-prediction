...
def main():
    ...
    study_popluation_csv_path =  os.path.join(tabular_dir, 'RS_CT_subcohort_population_linked_%s_excluded.csv' %category)
    ...
    if ascvd: # ASCVD (ASCVD mort, MI, stroke)
        ...
        processed_csv_path =  os.path.join(tabular_dir, 'RS_CT_subcohort_%s_outcomes_linked_%s_excluded.csv' %(argument,category))
    ...
    # ax.legend()
    fig.savefig(os.path.join(figure_dir, "%s_histogram_%s.png" %(argument,category)),  bbox_inches='tight', dpi = 300)
    ...
    fig.savefig(os.path.join(figure_dir, "age_distribution_%s.png" %category))
