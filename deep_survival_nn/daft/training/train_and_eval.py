...
def train_and_evaluate(
    ...
    for i in range(num_epochs):
        ...
        if evaluator is not None:
            ...
            if best_cindex < val_cindex:
                ...
                flag = 0
                # if i >= 2:
                torch.save(model, os.path.join(trained_model_dir, '%s_trained_network_%s_%s_%s_%s_%s.pth' %(discr_net, modal, outcome, category, gender, fold_i)))
                print ("Model saved")
            else:
                flag += 1         
                print ("flag: ", flag)           
    ...
    if evaluator is not None:
      ...
      plt.legend()
      plt.savefig(os.path.join(figure_dir, 'training_loss_curve_%s_%s_%s_%s_%s_%s.png' %(modal, category, discr_net, outcome, gender, fold_i)))
      ...
      plt.legend()
      plt.savefig(os.path.join(figure_dir, 'training_cindex_curve_%s_%s_%s_%s_%s_%s.png' %(modal, category, discr_net, outcome, gender, fold_i)))
