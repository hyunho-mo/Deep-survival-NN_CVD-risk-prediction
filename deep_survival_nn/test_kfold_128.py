...
def main(args=None):
    ...
    for i in range(k_of_fold):
        print ("k=", i)
        args.train_data = os.path.join(hdf_dir, "ergo_train_CHD_128_%s.hdf5" %i )
        args.val_data = os.path.join(hdf_dir, "ergo_val_CHD_128_%s.hdf5" %i )
        args.test_data = os.path.join(hdf_dir, "ergo_test_CHD_128_%s.hdf5" %i )
