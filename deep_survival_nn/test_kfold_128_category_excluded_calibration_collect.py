...
## Create dir
if not os.path.exists(calib_table_dir):
    os.makedirs(calib_table_dir)
...
def main(args=None):
    ...
    net_dir = os.path.join(category_dir, discr_net)
    ...
    for i in range(k_of_fold):
        ...
        calib_df["event"] = test_y["event"]
        calib_df["time"] = test_y["time"]

        ## convert logits (output) to partial hazard
        partial_hazard = np.exp(logits)
        partial_hazard_list = partial_hazard.tolist()


        partial_hazard_list_normalized = (partial_hazard - partial_hazard.min()) / (partial_hazard.max() - partial_hazard.min())

        calib_df["output"] = partial_hazard_list
        ...
        calib_table_filename = "table_calib_%s_%s_%s.csv" %(category,discr_net,i)
        calib_table_filepath = os.path.join(net_dir, calib_table_filename)
