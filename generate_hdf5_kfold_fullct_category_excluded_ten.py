...
def generate_datafiles(hdf5_filename, x_df, y_df, len_df, calc_dir, fold ):
    ...
    for index, id in enumerate(x_df["patient_name"].values):
        ...
        img_filepath = os.path.join(calc_dir, "fullct_%s.npy" %id)
...
def main():
    ...
    if crop:
        ...
        fullct_dir = os.path.join(storage_dir, 'fullct_array_%s_%s_crop' %(num_heartslice, resize))
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        if cac:
            ...
            generate_datafiles('DAFT/ergo_train_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i), X_train, y_train, len_train, fullct_dir, i)
            generate_datafiles('DAFT/ergo_val_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i), X_val, y_val, len_val, fullct_dir, i)
            generate_datafiles_test('DAFT/ergo_test_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i), X_test, y_test, len_test, fullct_dir, i)
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        if cac:
            generate_datafiles('DAFT/ergo_train_cac_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_train, y_train, len_train, heart_dir)
            generate_datafiles('DAFT/ergo_val_cac_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_val, y_val, len_val, heart_dir)
            generate_datafiles('DAFT/ergo_test_cac_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_test, y_test, len_test, heart_dir)
        else:
            generate_datafiles('DAFT/ergo_train_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_train, y_train, len_train, fullct_dir, i)
            generate_datafiles('DAFT/ergo_val_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_val, y_val, len_val, fullct_dir, i)
            generate_datafiles_test('DAFT/ergo_test_fullct_%s_%s_%s_excluded_%s.hdf5' %(outcome, resize, category, i+5), X_test, y_test, len_test, fullct_dir, i)
