...
def generate_datafiles(hdf5_filename, x_df, y_df, len_df, calc_dir, fold ):
    ...
    for index, id in enumerate(x_df["patient_name"].values):
        ...
        img_filepath = os.path.join(calc_dir, "calc_%s.npy" %id)
...
def main():
    ...
    if crop:
        ...
        calc_dir = os.path.join(storage_dir, 'calc_array_%s_%s_fp' %(num_heartslice, resize))
    ...
    tabular_filpath = os.path.join(tabular_dir, 'RS_CT_subcohort_time2event_%s_%s_linked_computed_cac.csv' %(outcome, imputation))
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        print (len(X_test))
        if cac:
            generate_datafiles('DAFT/ergo_train_cac_%s_%s_%s.hdf5' %(outcome, resize, i), X_train, y_train, len_train, calc_dir)
            generate_datafiles('DAFT/ergo_val_cac_%s_%s_%s.hdf5' %(outcome, resize, i), X_val, y_val, len_val, calc_dir)
            generate_datafiles('DAFT/ergo_test_cac_%s_%s_%s.hdf5' %(outcome, resize, i), X_test, y_test, len_test, calc_dir)
        else:
            generate_datafiles('DAFT/ergo_train_computed_%s_%s_%s.hdf5' %(outcome, resize, i), X_train, y_train, len_train, calc_dir, i)
            generate_datafiles('DAFT/ergo_val_computed_%s_%s_%s.hdf5' %(outcome, resize, i), X_val, y_val, len_val, calc_dir, i)
            generate_datafiles_test('DAFT/ergo_test_computed_%s_%s_%s.hdf5' %(outcome, resize, i), X_test, y_test, len_test, calc_dir, i)
    ...
    for i, (train_index, test_index) in enumerate(skf.split(X, y)):
        ...
        print (len(X_test))
        if cac:
            generate_datafiles('DAFT/ergo_train_cac_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_train, y_train, len_train, calc_dir)
            generate_datafiles('DAFT/ergo_val_cac_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_val, y_val, len_val, calc_dir)
            generate_datafiles('DAFT/ergo_test_cac_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_test, y_test, len_test, calc_dir)
        else:
            generate_datafiles('DAFT/ergo_train_computed_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_train, y_train, len_train, calc_dir, i)
            generate_datafiles('DAFT/ergo_val_computed_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_val, y_val, len_val, calc_dir, i)
            generate_datafiles_test('DAFT/ergo_test_computed_%s_%s_%s.hdf5' %(outcome, resize, i+5), X_test, y_test, len_test, calc_dir, i)
