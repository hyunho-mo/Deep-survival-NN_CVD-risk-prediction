...
processed_csv_path =  os.path.join(tabular_dir, "RS_CT_subcohort_population.csv")
...
def main():
    ...
    print ("total number of participants scan age less than 85: ", len(template_df))
    ...
    print ("after prevalent", len(template_df))
    intervention_history_cols = ["prev_CVATIA", "prev_AAA_PAD_CEA_OK"]
    # ## Only exclude if prevalent (‘1’)
    for hist_col in intervention_history_cols:
         template_df = template_df.loc[(template_df[hist_col]!=1)] 
    print ("after intervention", len(template_df))
